...
Quick Start:
    traj = trajGoldenAngle(N=128, n_spokes_per_slice=360, nz=64)
    traj_staggered = trajGoldenAngle(N=128, n_spokes_per_slice=360, nz=64, staggered=True)
"""

import numpy as np
import lib.radial

def trajGoldenAngle(N, n_spokes_per_slice, nz, staggered=False):
    """
    ...
    for i, pez in enumerate(pez_lines):
        ...
        tmp_slice_traj = lib.radial.trajGoldenAngle(N, n_spokes_per_slice, starting_angle=starting_angle)
